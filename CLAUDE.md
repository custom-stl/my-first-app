...
| きめごと | あたい | なぜ |
| --- | --- | --- |
| `LIMIT_DEFAULT` | 15ふん | なにも きめて いない 人は これ。**人ごと**（`sn-limit`） |
| `REST_DEFAULT` / `REST_CHOICES` | 5ふん / 3・5・10・15 | きゅうけいの ながさ。**みんな 共通**（`sn-rest`）。休むのに 人ごとの 理由が ない |
...
- `maybeRest({ hard, force })` が 出すか どうかを きめる。
  - **`inQuestion()` の あいだは 出さない**（1もん といてから きりかわる。
    `GRACE_MIN` を こえたら とちゅうでも 出す）
  - **`inPractice()`（`#quiz`/`#tyquiz`/`#result`/`#tyresult`/`#rest`）の ときだけ きりかえる。**
    トップと メニューでは きりかえない ―― **きゅうけい中でも ほかの人が えらべる** ように する ため。
    といた はじめ（`startSet`/`tyStart`）と ひらいた ときだけ `hard: true` で きりかえる
  - **`inGrownUp()`（きろく・みんな・ひとりぶん）でも 出さない** ―― 出すと おとなが せっていを
    かえられなく なる
- **きゅうけい中でも ほかの人に かわれる**（きょうだいで 1だいを つかう ため）。
  左上の だいめいラベル（`#to-top`）と きゅうけい画面の「ほかの 人が やる」（`#rest-other`）が
  `openTop()` を よぶ。トップには `renderRestNote()` が
  「はるとは きゅうけい中（あと 3:59）。ほかの 人は えらべば できるよ。」と 出す（がめんは かえない）。
  ほかの人を えらべば そのまま できる。はるとに もどして はじめようと すると また とまる
